...
type predicate = syntactic_only expr

and 'a funsig = Funsig of ident * ident list * 'a expr

and 'a expr =
  | Int : int -> 'a expr 
  | Bool : bool -> 'a expr
  | Var : ident -> 'a expr
  | Function : (ident list * 'a expr) -> 'a expr 
  | Input : 'a expr
  | Appl : ('a expr * 'a expr) -> 'a expr
  | Let : (ident * 'a expr * 'a expr) -> 'a expr
  | LetRecFun : ('a funsig list * 'a expr) -> 'a expr  
  | LetFun : ('a funsig * 'a expr) -> 'a expr  
  | LetWithType : (ident * 'a expr * 'a expr * 'a expr) -> 'a syntactic_and_semantic expr
  | LetRecFunWithType : ('a funsig list * 'a expr * 'a expr list) -> 'a syntactic_and_semantic expr
  | LetFunWithType : ('a funsig * 'a expr * 'a expr) -> 'a syntactic_and_semantic expr
  | Plus : ('a expr * 'a expr) -> 'a expr 
  | Minus : ('a expr * 'a expr) -> 'a expr 
  | Times : ('a expr * 'a expr) -> 'a expr 
  | Divide : ('a expr * 'a expr) -> 'a expr 
  | Modulus : ('a expr * 'a expr) -> 'a expr 
  | Equal : ('a expr * 'a expr) -> 'a expr 
  | Neq : ('a expr * 'a expr) -> 'a expr 
  | LessThan : ('a expr * 'a expr) -> 'a expr 
  | Leq : ('a expr * 'a expr) -> 'a expr 
  | GreaterThan : ('a expr * 'a expr) -> 'a expr 
  | Geq : ('a expr * 'a expr) -> 'a expr 
  | And : ('a expr * 'a expr) -> 'a expr 
  | Or : ('a expr * 'a expr) -> 'a expr 
  | Not : 'a expr -> 'a expr 
  | If : ('a expr * 'a expr * 'a expr) -> 'a expr 
  | Record : ('a expr Ident_map.t) -> 'a expr 
  | RecordProj : ('a expr * label) -> 'a expr
  | Match : ('a expr * (pattern * 'a expr) list) -> 'a expr
  | VariantExpr : (variant_label * 'a expr) -> 'a expr
  | List : 'a expr list -> 'a expr 
  | ListCons : ('a expr * 'a expr) -> 'a expr 
  (* TODO: Create a separate class of constructors for type errors? *)
  | TypeError : ident -> 'a expr
  | Assert : 'a expr -> 'a expr 
  | Assume : 'a expr -> 'a expr 
  | Untouched : string -> 'a expr
  (* Type expressions *)
  | TypeVar : ident -> syntactic_only expr
  | TypeInt : syntactic_only expr
  | TypeBool : syntactic_only expr
  | TypeRecord : (syntactic_only expr) Ident_map.t -> syntactic_only expr
  | TypeList : syntactic_only expr -> syntactic_only expr
  | TypeArrow : (syntactic_only expr * syntactic_only expr) -> syntactic_only expr
  | TypeArrowD : ((ident * syntactic_only expr) * syntactic_only expr) -> syntactic_only expr
  | TypeSet : (syntactic_only expr) * predicate -> syntactic_only expr
  | TypeUnion : (syntactic_only expr * syntactic_only expr) -> syntactic_only expr
  | TypeIntersect : (syntactic_only expr * syntactic_only expr) -> syntactic_only expr
  | TypeRecurse : (ident * syntactic_only expr) -> syntactic_only expr
...
let rec equal_funsig: type a. a funsig -> a funsig -> bool =
        ...
        (id1 = id2) && (List.eq equal_ident params1 params2) && (equal_expr fe1 fe2)
...
and equal_expr: type a. a expr -> a expr -> bool = 
  ...
  (* | Var _, _ -> false *)
  | List l1, List l2 -> 
    List.eq equal_expr l1 l2
  (* | List _, _ -> false *)
  | Record r1, Record r2 -> 
    Ident_map.equal equal_expr r1 r2
  (* | Record _, _ -> false *)
  | Untouched s1, Untouched s2 -> s1 = s2
  (* | Untouched _, _ -> false *)
  | Function (id_lst1, fun_body1), Function (id_lst2, fun_body2) -> 
    (List.eq equal_ident id_lst1 id_lst2) && (equal_expr fun_body1 fun_body2)
  (* | Function _, _ -> false *)
  | Let (x1, xe1, e1), Let (x2, xe2, e2) ->
    (x1 = x2) && (equal_expr xe1 xe2) && (equal_expr e1 e2)
  (* | Let _, _ -> false *)
  | LetFun (f1, e1), LetFun (f2, e2) -> 
    (equal_funsig f1 f2) && (equal_expr e1 e2)
  (* | LetFun _, _ -> false *)
  ...
  | LetWithType (x1, xe1, e1, t1), LetWithType (x2, xe2, e2, t2) ->
    (x1 = x2) && (equal_expr xe1 xe2) && 
    (equal_expr e1 e2) && (equal_expr t1 t2)
  ...
  | ListCons (lop1, rop1), ListCons (lop2, rop2) ->
    (equal_expr lop1 lop2) && 
    (equal_expr rop1 rop2) 
  (* | Or _, _
  | And _, _
  | Equal _, _
  | Neq _, _
  ...
  | TypeRecord t1, TypeRecord t2 -> 
    Ident_map.equal equal_expr t1 t2
  ...
  | TypeSet (lt1, rt1), TypeSet (lt2, rt2) ->
    (equal_expr lt1 lt2) && (equal_expr rt1 rt2)
...
and compare_expr : type a. a expr -> a expr -> int = 
  fun e1 e2 ->
    ...
    | List l1, List l2 -> 
      List.compare compare_expr l1 l2
    | Record r1, Record r2 -> 
    ...
    | TypeRecord t1, TypeRecord t2 -> 
      Ident_map.compare compare_expr t1 t2
    | TypeList t1, TypeList t2 ->
      compare_expr t1 t2
    | TypeArrow (lt1, rt1), TypeArrow (lt2, rt2) 
    | TypeUnion (lt1, rt1), TypeUnion (lt2, rt2)
