...
let rec build_recursive_type (t_var : ident) (e_desc : syntactic_only expr_desc) = 
  let e = e_desc.body in
  let tag = e_desc.tag in
  let body' = 
    match e with
    | Int _ | Bool _ | TypeError _ | Untouched _ | Input -> e
    ...
    | Assume e_desc ->
      let e_desc' = build_recursive_type t_var e_desc in
      Assume e_desc'
    | TypeVar _ | TypeInt | TypeBool | TypeUntouched _ -> e
    | TypeRecord r ->
      let r' =
        Ident_map.map
          (fun e_desc -> build_recursive_type t_var e_desc)
